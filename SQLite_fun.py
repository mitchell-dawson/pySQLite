...
def create_table(conn, create_table_sql):
	...
	@param create_table_sql: a CREATE TABLE statement - see example create_database.py script
	"""
...
def update(conn, table, column_tuple, value_tuple, id_table, id_num):
	...
	if len(column_tuple) == 1:
		...
		sql = "UPDATE %s SET %s=? WHERE %s = %s" %(table, column_tuple[0], id_table,id_num)
